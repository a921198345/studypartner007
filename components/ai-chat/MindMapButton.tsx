...
export function MindMapButton({ message, answer }: MindMapButtonProps) {
  ...
  const detectSubject = (text: string): string => {
    ...
    const subjectKeywords = {
      'civil': ['民法', '合同', '物权', '债权', '侵权', '婚姻', '继承', '人格权', '担保', '买卖', '租赁', '借贷', '赠与', '委托', '无因管理', '不当得利', '离婚', '财产分割', '子女抚养'],
      'criminal': ['刑法', '犯罪', '刑罚', '故意', '过失', '正当防卫', '紧急避险', '犯罪构成', '主观', '客观', '共犯', '量刑', '死刑', '有期徒刑', '罚金', '诈骗', '盗窃', '抢劫', '贪污', '受贿'],
      'administrative': ['行政法', '行政许可', '行政处罚', '行政强制', '行政复议', '行政诉讼', '政府', '行政机关', '公务员', '行政行为'],
      'constitutional': ['宪法', '基本权利', '国家机构', '立法', '司法', '行政权', '公民权利', '选举', '人大', '国务院'],
      'civilProcedure': ['民事诉讼', '民诉', '原告', '被告', '起诉', '答辩', '证据', '判决', '执行', '管辖', '送达', '期限', '上诉', '再审'],
      'criminalProcedure': ['刑事诉讼', '刑诉', '侦查', '起诉', '审判', '辩护', '证据', '强制措施', '逮捕', '拘留', '取保候审'],
      'economic': ['经济法', '公司法', '证券', '票据', '保险', '破产', '反垄断', '反不正当竞争', '消费者', '产品质量'],
      'commercial': ['商法', '公司', '股东', '董事', '监事', '股权', '并购', '融资', '上市', '证券', '票据', '保险', '海商', '破产'],
      'international': ['国际法', '国际公法', '国际私法', '国际经济法', '条约', '国际组织', '外交', '领事', '国际贸易', 'WTO'],
      'theory': ['法理学', '法律原理', '法的概念', '法律关系', '法律体系', '法治', '立法', '司法', '执法', '法律解释']
    }
    ...
    // 找出得分最高的科目
    let maxScore = 0
    let detectedSubject = 'civil' // 默认民法
  ...
  const handleNavigateToMindMap = async () => {
    ...
    try {
      ...
      toast({
        description: `正在跳转到${subjectNames[subject] || '民法'}知识导图，关键词：${keywords}`,
        duration: 2000,
      })
