...
export const questionApi = {
  ...
  async searchWithMultipleKeywords(params = {}) {
    try {
      ...
      const hasAuth = authHeaders['x-user-token'] || authHeaders['Authorization'];
      
      // 根据认证状态选择不同的API端点
      const apiEndpoint = hasAuth ? '/api/exams/questions/search' : '/api/exams/questions/search-public';
      ...
      if (!response.ok) {
        ...
        // 如果是401错误，尝试使用公开搜索
        if (response.status === 401 && apiEndpoint !== '/api/exams/questions/search-public') {
          console.log('认证失败，尝试使用公开搜索API');
          const publicResponse = await fetch('/api/exams/questions/search-public', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
      ...
      console.log('多关键词搜索响应:', {
        ...
        isPublicSearch: data.data?.searchInfo?.isPublicSearch
      });
