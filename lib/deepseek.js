...
export function buildPrompt(question, context = [], imageBase64 = null) {
  ...
  const systemPrompt = `你是一位深受学生喜爱的法考辅导专家，擅长用费曼学习法帮助学生理解复杂的法律概念。你的教学理念是"用最简单的语言解释最复杂的法律"。
...
let globalRequestTracker = {
  ...
  async checkLimit() {
    ...
    console.log(`当前请求计数: ${this.requests.length}/${this.maxRequests}`);
    
    if (this.requests.length >= this.maxRequests) {
      const oldestRequest = Math.min(...this.requests);
      const waitTime = this.windowMs - (now - oldestRequest) + 1000; // 额外加1秒缓冲
      console.log(`⏰ API请求频率限制，需等待 ${Math.ceil(waitTime / 1000)} 秒`);
      throw new Error(`API请求频率过高，请等待 ${Math.ceil(waitTime / 1000)} 秒后重试`);
    }
    
    this.requests.push(now);
    console.log(`✅ 请求通过，已记录时间戳`);
    return true;
  }
};
...
export async function generateAnswer(prompt) {
  try {
    ...
    // 检查请求频率限制
    await globalRequestTracker.checkLimit();
...
/**
 * 获取DeepSeek API的简单文本回答（用于学习计划生成）
 * @param {string} prompt - 提示词
 * @returns {Promise<string>} - 生成的文本
 */
export async function getDeepSeekCompletion(prompt) {
  try {
    const apiKey = process.env.DEEPSEEK_API_KEY;
    if (!apiKey) {
      throw new Error('未设置DEEPSEEK_API_KEY环境变量');
    ...
    console.log('调用DeepSeek API生成学习计划...');
