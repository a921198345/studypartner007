...
def parse_question(text):
    ...
    for pattern in option_patterns:
        matches = re.findall(pattern, question_part, re.DOTALL | re.MULTILINE)
        for opt, content in matches:
            content = clean_text(content)
            if content and opt not in options_dict:
...
def main():
    parser = argparse.ArgumentParser(description='Web上传专用的法考题目解析(处理特殊格式)')
