...
export async function POST(request) {
  ...
  try {
    ...
    if (parseResult.questions && parseResult.questions.length > 0) {
      ...
      for (const question of parseResult.questions) {
        try {
          ...
          if (existing.length > 0) {
            // 更新现有题目
            console.log(`更新题目 ${question.question_id}`);
            ...
            // 插入新题目
            console.log(`插入新题目 ${question.question_id}, 类型: ${question.question_type} -> ${question.question_type === 'multiple' ? 2 : 1}`);
            ...
            console.log(`插入结果:`, insertResult.insertId, insertResult.affectedRows);
            savedCount++;
          }
        } catch (dbError) {
      ...
      console.log('数据库中现有题目总数:', countResult[0].total);
      ...
      console.log(`${subject}科目题目数:`, verifyResult[0].count);
      
      console.log(`操作完成: 新增 ${savedCount} 个，更新 ${updateCount} 个，失败 ${errorCount} 个`);
      ...
      // 构建返回消息
      let successMessage = `文件上传并解析成功，新增 ${savedCount} 个题目，更新 ${updateCount} 个题目`;
      if (errorCount > 0) {
        successMessage += `，${errorCount} 个题目保存失败`;
      }
      ...
      return NextResponse.json({
        ...
        updated_questions: updateCount,
        failed_questions: errorCount,
        database_total: countResult[0].total,
