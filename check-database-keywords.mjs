...
async function checkKeywordDistribution() {
  ...
  try {
    ...
    const [totalResult] = await connection.execute(
      "SELECT COUNT(*) as count FROM questions WHERE question_text LIKE ? OR options_json LIKE ?",
      ['%盗窃%', '%盗窃%']
    );
    ...
    const [subjectStats] = await connection.execute(
      `SELECT subject, COUNT(*) as count 
       FROM questions 
       WHERE (question_text LIKE ? OR options_json LIKE ?)
       GROUP BY subject
       ORDER BY count DESC`,
      ['%盗窃%', '%盗窃%']
    );
    ...
    const [preciseResult] = await connection.execute(
      `SELECT COUNT(*) as count FROM questions 
       WHERE (question_text LIKE ? OR options_json LIKE ?)
       AND (question_text REGEXP ? OR options_json REGEXP ?)`,
    ...
    for (const subject of subjects) {
      const [examples] = await connection.execute(
        `SELECT id, subject, SUBSTRING(question_text, 1, 100) as content_preview
         FROM questions 
         WHERE subject = ? AND (question_text LIKE ? OR options_json LIKE ?)
         LIMIT 2`,
        [subject, '%盗窃%', '%盗窃%']
      );
