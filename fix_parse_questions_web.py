...
def parse_question(text):
    ...
    option_positions = []
    for opt in ['A', 'B', 'C', 'D']:
        match = re.search(rf'{opt}\.', question_part)
        if match:
            option_positions.append((opt, match.start()))
    
    # 按位置排序
    option_positions.sort(key=lambda x: x[1])
    
    # 提取每个选项的内容
    for i, (opt, start_pos) in enumerate(option_positions):
        # 找到选项开始位置（跳过"A."）
        option_start = question_part.find(f'{opt}.', start_pos) + 2
        
        # 找到选项结束位置（下一个选项的开始或题目部分的结束）
        if i < len(option_positions) - 1:
            option_end = option_positions[i + 1][1]
        else:
            option_end = len(question_part)
        
        # 提取选项内容
        option_content = question_part[option_start:option_end].strip()
        option_content = clean_text(option_content)
        
        if option_content:
            options_dict[opt] = option_content
